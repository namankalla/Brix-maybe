...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    (material.uniforms.uFrequency.value as number) = frequency;
    (material.uniforms.uWarpStrength.value as number) = warpStrength;
    (material.uniforms.uMouseInfluence.value as number) = mouseInfluence;
    (material.uniforms.uParallax.value as number) = parallax;
    (material.uniforms.uNoise.value as number) = noise;
    ...
    const arr = (colors || []).filter(Boolean).slice(0, MAX_COLORS as number).map(toVec3);
    for (let i = 0; i < (MAX_COLORS as number); i++) {
      const vec = (material.uniforms.uColors.value as THREE.Vector3[])[i];
      if (i < arr.length) vec.copy(arr[i]);
      else vec.set(0, 0, 0);
    }
    (material.uniforms.uColorCount.value as number) = arr.length;

    (material.uniforms.uTransparent.value as number) = transparent ? 1 : 0;
